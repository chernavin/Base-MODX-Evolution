...
function startComplete() {
    ...
    var cur = myCodeMirror.getCursor(false), token = myCodeMirror.getTokenAt(cur), tprop = token;
    ...
    while (tprop.className == "js-property") {
        tprop = myCodeMirror.getTokenAt({line: cur.line, ch: tprop.start});
    ...
    function insert(str) {
        myCodeMirror.replaceRange(str, {line: cur.line, ch: token.start}, {line: cur.line, ch: token.end});
    }
    ...
    connect(sel, "keydown", function(event) {
        ...
        // Escape
        else if (code == 27) {event.stop(); close(); myCodeMirror.focus();}
        else if (code != 38 && code != 40) {close(); myCodeMirror.focus(); setTimeout(startComplete, 50);}
    });
