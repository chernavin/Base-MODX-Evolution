    ...
    if ($isPWDActivate==1){
        ...
        if($limit==1) {
            ...
            if($newpwdkey!=$pwdkey) {
                ...
                $output = webLoginAlert($langTXT[0],$alerttpl);
            ...
            if(!$ds || !$ds2) $output = webLoginAlert($langTXT[1],$alerttpl);

            else if(!$pwdActId) $output = webLoginAlert($langTXT[2],$alerttpl);
            ...
            $output = webLoginAlert($langTXT[3],$alerttpl);
    ...
    if ($isPWDReminder==1) {
        ...
        if($limit==1) {
            ...
            if(!$sent) {
                ...
                $output =  webLoginAlert($langTXT[4],$alerttpl);
            ...
            */
            ...
            sendMail($langTXT[29],$email,$message);
            
            

            if(!$pwdReqId) $output = webLoginAlert($langTXT[5],$alerttpl);
            ...
            $output = webLoginAlert($langTXT[6],$alerttpl);
...
# process login
    ...
    if($limit==0 || $limit>1) {
        ...
        $output = webLoginAlert($langTXT[7],$alerttpl);
    ...
    if($failedlogins>=$modx->config['failed_login_attempts'] && $blockeduntildate>time()) {    // blocked due to number of login errors.
        ...
        $output = webLoginAlert($langTXT[8],$alerttpl);
    ...
    if($blocked=="1") { // this user has been blocked by an admin, so no way he's loggin in!
        ...
        $output = webLoginAlert($langTXT[9],$alerttpl);
    ...
    if($blockeduntildate>time()) { // this user has a block until date
        ...
        $output = webLoginAlert($langTXT[10],$alerttpl);
    ...
    if($blockedafterdate>0 && $blockedafterdate<time()) { // this user has a block after date
        ...
        $output = webLoginAlert($langTXT[11],$alerttpl);
    ...
    if (isset($modx->config['allowed_ip'])) {
        ...
        if (strpos($modx->config['allowed_ip'],$_SERVER['REMOTE_ADDR'])===false) {
            ...
            $output = webLoginAlert($langTXT[12],$alerttpl);
    ...
    if (isset($modx->config['allowed_days'])) {
        ...
        if (strpos($modx->config['allowed_days'],"$day")===false) {
            ...
            $output = webLoginAlert($langTXT[13],$alerttpl);
