...
if(!$isPostBack){
    ...
    if($username=="") {
        ...
        $output = webLoginAlert($langTXT[17],$alerttpl).$tpl;
        ...
        if(!$rs = $modx->db->query($sql)){
            ...
            $output = webLoginAlert($langTXT[18]." $username.",$alerttpl).$tpl;
        ...
        if($limit>0) {
            ...
            $output = webLoginAlert($langTXT[19],$alerttpl).$tpl;
    ...
    if($email=='' || !preg_match("/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,6}$/i", $email)){
        ...
        $output = webLoginAlert($langTXT[20],$alerttpl).$tpl;
    ...
    if(!$rs = $modx->db->query($sql)){
        ...
        $output = webLoginAlert($langTXT[21].$email,$alerttpl).$tpl;
    ...
    if($limit>0) {
        ...
        if($row['internalKey']!=$id) {
            ...
            $output = webLoginAlert($langTXT[22],$alerttpl).$tpl;
 	...
 	if (isset($_POST['password'])) { 	  	 
		...
		// verify password 	  	 
 	    ...
 	    if ($_POST['password'] != $_POST['confirmpassword']) { 	  	 
 	  		...
 	  		$output = webLoginAlert($langTXT[23],$alerttpl). $tpl; 	  	 
	    ...
	    if (strlen($password) < 6 ) {
	        ...
	        $output = webLoginAlert($langTXT[24],$alerttpl).$tpl;
	    ...
	    elseif($password=="") {
	        ...
	        $output = webLoginAlert($langTXT[25],$alerttpl).$tpl;
